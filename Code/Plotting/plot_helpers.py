...
def plot_counts(df,column,startDate = '2001-01-01 00:00:00',endDate = '2021-01-01 00:00:00'):
    dfsub = df.loc[startDate:endDate].groupby('date').count()
    plt.figure(figsize = (15,10))
    plt.title('# of Pixels Containing Observations: ' + column[:-5], fontsize = 40)
...
def plot_timeseries(gdf,pix_ids,column,pix_id_col='pix_id',startDate = '2001-01-01 00:00:00',endDate = '2021-01-01 00:00:00'):
    gdfsub = gdf[gdf[pix_id_col].isin(pix_ids)].groupby([pix_id_col,'date'])[column].mean()
...
def map_counts(df,column,startDate='2001-01-01',endDate=None,extents =  [-77.458841,  -74.767094, 36.757802, 39.920274],num=5):
    
    if endDate == None:
        endDate = startDate 
    
    dfsub = df.loc[startDate:endDate].groupby('pix_id').aggregate(['mean','count'])
    ...
    ax = plt.scatter(dfsub.longitude['mean'],dfsub.latitude['mean'] ,s = dfsub[column]['count'],zorder=2)
    handles, labels = ax.legend_elements(prop="sizes", num=num,  markerfacecolor='none', markeredgecolor='k')
    ...
    ax.scatter(dfsub.longitude['mean'],dfsub.latitude['mean'],s = dfsub[column]['count'],zorder=2, facecolors='none', edgecolors='k')
    ax.legend(handles[::int(num/4)], labels[::int(num/4)], loc="lower right", title="Number of \n Observations",handletextpad=3.5,labelspacing=1.5,borderpad=3)
    plt.title('Total Number of Observations: ' + column)
    
    
    
def map_variable(df,column,startDate = '2008-04-17',endDate = None,num=5,extents =  [-77.458841,  -74.767094, 36.757802, 39.920274]):
    ...
    plt.figure(figsize=[15,15])
     # create dummy plot in order to get information for creating the legend   
    ax = plt.scatter(dfsub.longitude,dfsub.latitude ,c = dfsub[column],zorder=2)
