...
def getLonLat(feat):
    ...
    Asssumes the geometry is in the form of a Point().'''
...
def setGeometry(feat):  
    ...
    Assumes the latitude and longitude are points '''
...
def getValuesMean(nestedDict,colSelectors,groupName):
    ...
    operators though some (such as the count operator) have ouput different 
...
stations = ee.FeatureCollection('users/jonschwenk/del_chk_insitu_obs').map(formatDate).map(getLonLat)
